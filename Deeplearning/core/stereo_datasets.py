...
class Middlebury(StereoDataset):
    def __init__(self, aug_params=None, root='/data/Middlebury', split='F'):
        ...
        # lines = list(filter(lambda p: any(s in p.split('/') for s in Path(os.path.join(root,
        # "MiddEval3/official_train.txt")).read_text().splitlines()), lines)) image1_list = sorted([os.path.join(
        # root, "MiddEval3", f'training{split}', f'{name}/im0.png') for name in lines]) image2_list = sorted([
        # os.path.join(root, "MiddEval3", f'training{split}', f'{name}/im1.png') for name in lines]) disp_list =
        # sorted([os.path.join(root, "MiddEval3", f'training{split}', f'{name}/disp0GT.pfm') for name in lines])
